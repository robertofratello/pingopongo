...
class EloRepo:
    ...
    def register_match(self, winner, loser, db, timestamp=None, clear_undo=True):
        ...
        matches_db.write(winner, loser, timestamp, json.dumps(winner_row), json.dumps(loser_row))
    ...
    def update_multiplier(row, timestamp):
        ...
        years_passed = (int(timestamp) - int(row["last_played"])) / (86400 * 365)
    ...
    def redo(self, db): 
        ...
        res = self.register_match(last_row[WINNER], last_row[LOSER], db, timestamp =last_row[TIME], clear_undo=False)
        _, matches_db = self.get_dbs(db)
